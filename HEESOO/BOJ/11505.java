...
//		Arrays.fill(tree, 1);	
	...
	public static long update(int node, int left, int right, int idx, int origin, int c) {
		if(idx<left || idx>right) return tree[node];		
		if(left==right) return tree[node]=c;
		
		int mid=(left+right)/2;
		return tree[node]=update(node*2, left, mid, idx, origin, c)*update(node*2+1, mid+1, right, idx, origin, c)%MOD;
	}
